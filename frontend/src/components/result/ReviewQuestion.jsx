    ...
    <div className={clsx(
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-grow">
          <h4 className="font-medium text-gray-900 dark:text-white mb-2">
            {t('review.questionLabel', { number: index + 1 })}: <span className="font-normal text-gray-700 dark:text-white/90">{questionText}</span>
          </h4>
        </div>
      </div>
      ...
      {!isCorrect && correctOption.explanation && (
        <div className="mt-4 ml-9 p-4 bg-white dark:bg-white/5 rounded-lg border border-gray-200 dark:border-white/10 text-sm text-gray-600 dark:text-white/80">
          <span className="font-bold text-primary-600 dark:text-primary-300">{t('review. explanation')}:</span> {correctOption.explanation}
        </div>
      )}
    </div>
  );
